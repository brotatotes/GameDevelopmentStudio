...
public class continuousHitbox : MonoBehaviour {
	List<Movement> overlappingControl = new List<Movement> (); 
	...
	void Update () {
		if (hitboxDuration > 0.0f || !timedHitbox) {
			foreach(Movement cont in overlappingControl) {
	...
	internal void OnTriggerEnter2D(Collider2D other) {
		...
		if (other.gameObject.GetComponent<Movement>()) {
			overlappingControl.Add (other.gameObject.GetComponent<Movement> ()); //Adds the other object's Controller2D to list of contacting objects
		}
	} 
	internal void OnTriggerExit2D(Collider2D other) {
		//Debug.Log ("Collision ended with Continuous hitbox");
		if (other.gameObject.GetComponent<Movement> ()) {
			overlappingControl.Remove (other.gameObject.GetComponent<Movement> ()); //Removes the object from the list
		}
	}
}
